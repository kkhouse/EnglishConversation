package com.example.englishassistantapp.domain.network.internal.extension

import com.example.englishassistantapp.domain.network.internal.JsonLenient
...
internal suspend inline fun <reified T> FlowCollector<T>.streamEventsFrom(response: HttpResponse) {
    ...
    while (!channel.isClosedForRead) {
        ...
        val value: T = when {
            ...
            line.startsWith(STREAM_PREFIX) -> JsonLenient.decodeFromString(line.removePrefix(
                STREAM_PREFIX
            ))
            else -> continue
        }
